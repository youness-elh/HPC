...
void memory_bound(){
    ...
    timer1.stop();
    const long int jumpSize = 64*sizeof(char);
    Timer timer8;
    // iterate from 0 to size by step jumSize and update arr[i] *= 3
    long int i = 0;
    while(i < size){
        arr[i] *= 3;
        i+=jumpSize;
...
void misprediction(){
    ...
    Timer timer1024;
    {
        ...
        for(long int repeat = 0 ; repeat < nbRepeat ; ++repeat){
            for (long int i = 0; i < size; i++){
                // using the last 3 times if history test
                if (i%1024!=0){
                    a += arr[i];
                }
                else{
    ...
    Timer timer3;
    {
        ...
        for(long int repeat = 0 ; repeat < nbRepeat ; ++repeat){
            for (long int i = 0; i < size; i++){
                // wrong history test. unable to use the last 3 times if history test 
                if (i%3!=0){
                    a += arr[i];
                }
                else{
...
void falsesharing(){
    ...
    #pragma omp parallel num_threads(nbThreads)
    {
        ...
        volatile long int& val = arr[omp_get_thread_num()*maxIncrement];
    ...
    #pragma omp parallel num_threads(nbThreads)
    {
        ...
        volatile long int& val = arr[omp_get_thread_num()];
...
void cache_effect(){
    const long int nbLongIntInCacheLine = 64/sizeof(long int);
